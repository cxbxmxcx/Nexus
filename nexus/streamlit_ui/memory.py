...
from nexus.streamlit_ui.embeddings import get_agent_engine, view_embeddings


def add_memory_to_store(nexus, memory_store):
    if memory_store is None:
        st.error("Please create a memory store first.")
        st.stop()

    agent_engine = get_agent_engine(nexus, "add", "memory")

    memory = st.text_area("Enter a memory to add to the store:")

    if st.button("Add Memory") and memory != "" and memory is not None:
        nexus.load_memory(memory_store, memory, agent_engine)
...
def memory_page(username, win_height):
    ...
    with st.sidebar.expander("Manage Memory Stores"):
        ...
        if st.button(
            ...
            disabled=(
                store_name == ""
                or store_name in nexus.get_memory_store_names()
                or store_name is None
                or len(store_name) < 3
            ),
        ):
            nexus.add_memory_store(store_name)
            st.success(f"Memory Store '{store_name}' created!")

        selected_store_to_delete = st.selectbox(
            "Select a memory store to delete:",
            options=list(nexus.get_memory_store_names()),
        )
        if st.button("Delete Memory Store"):
            nexus.delete_memory_store(selected_store_to_delete)
    ...
    with config_tabs[4]:
        ...
        memory_function = nexus.get_memory_function(memory_store.memory_type)
        ...
        if st.button("Save Configuration"):
            nexus.update_memory_store(memory_store)
